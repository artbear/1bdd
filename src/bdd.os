...
Процедура ДобавитьКомандуExec(Знач Парсер)
	...
	Парсер.ДобавитьПараметрФлагКоманды(Команда, "-fail-fast", "Немедленное завершение выполнения на первом же не пройденном сценарии");
	Парсер.ДобавитьИменованныйПараметрКоманды(Команда, "-junit-out", " -junit-out <путь-файла-отчета> - выводить отчет тестирования в формате JUnit.xml
...
Процедура ДобавитьОбщиеПараметрыКоманд(Парсер, Команда)
	...
	Парсер.ДобавитьИменованныйПараметрКоманды(Команда, "-debug", "-debug <on|off> - включает режим отладки (полный лог + остаются временные файлы)");
...
Функция ВыполнитьФичу(Знач ПутьФичи, Знач ПутьКБиблиотекам, Знач ИспользоватьБыстрыйОстановНаОшибке, Знач ИмяЭлементаСценария,
	...
	Если РезультатыВыполнения.Строки.Количество() > 0 Тогда
		...
		ИсполнительБДД.ВывестиИтоговыеРезультатыВыполнения(МассивШагов, СтруктураИтогов, ФайлФичи.ЭтоКаталог(), СтатусВыполнения);
	КонецЕсли;
...
Процедура ВывестиСправкуПоКоманде(Знач Команда) Экспорт
	Парсер = СоздатьПарсерКоманднойСтроки();
