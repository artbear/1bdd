...
Лог.Ошибка("Счетчик в цикле %1, кол-во строк %2", Счетчик, СтрокиДерева.Количество() -1);
		СтрокаДерева = СтрокиДерева[Счетчик];
		Лог.Ошибка("	Лексема <%1>, Тело <%2>, уровень <%3>, уровень() %4", СтрокаДерева.Лексема, СтрокаДерева.Тело, СтрокаДерева.УровеньЛексемы, СтрокаДерева.Уровень());
...
Лог.Ошибка(СтрШаблон("Вставляю контекст для строки <%1>, контекст <%2>", СтрокаСценария.Тело, УзелКонтекста.Тело));
