...
Процедура ЗаполнитьСтрокуТаблицы(Знач МассивЗначений, НоваяСтрока, Знач НомерСтроки)
	Для к = 0 По МассивЗначений.Количество() - 1 Цикл
		...
		Если НайденныеПараметры.Количество() = 1  Тогда
			...
			НашлиПараметр = (НайденныйПараметр.Тип = ВозможныеКлючиПараметров.Число И ЭтоЧисло(ЗначениеПараметра))
				Или НайденныйПараметр.Тип = ВозможныеКлючиПараметров.Дата;
...
Функция СоздатьРегулярныеВыражения()
	...
	Рез.Вставить("ЧислоИлиСловоСЧислом", Новый РегулярноеВыражение("(\d+(,\d+){0,1})|(-?\d+(,\d){0,1})+"));
