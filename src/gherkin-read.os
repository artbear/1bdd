...
Функция ПрочитатьФайлСценария(Знач ФайлСценария) Экспорт
	...
	СтруктураФичи = РазобратьТекстФичи(Фича, ФайлСценария.ПолноеИмя);
...
//
Функция ПрочитатьТекстФичи(Знач ТекстФичи, Знач ПутьФайла = "") Экспорт

	Лог.Отладка("Читаю текст фичи");
	Фича = Новый ЧитательСтрокФайла;
	Фича.ИнициализироватьИзСтроки(ТекстФичи);

	СтруктураФичи = РазобратьТекстФичи(Фича, ПутьФайла);
...
Функция СоздатьДеревоФич() Экспорт
	...
	Рез.Колонки.Добавить("ПутьФайла");
	Возврат Рез;
КонецФункции
...
Функция РазобратьТекстФичи(Знач Фича, Знач ПутьФайла)
	...
	ДеревоФич = ПолучитьДеревоФич(НайденныеЛексемы, ПутьФайла);
...
Функция ПолучитьДеревоФич(НайденныеЛексемы, Знач ПутьФайла)
	ДеревоФич = СоздатьДеревоФич();
	Индекс = 0;
	СтрокиДерева = ДеревоФич.Строки;

	ЗаполнитьДеревоФич(НайденныеЛексемы, СтрокиДерева, Индекс, ПутьФайла);
...
Процедура ЗаполнитьДеревоФич(НайденныеЛексемы, СтрокиДерева, Индекс, Знач ПутьФайла)
	...
	Пока Индекс < НайденныеЛексемы.Количество() Цикл
		...
		НоваяСтрока.ПутьФайла = ПутьФайла;
		...
		ИначеЕсли НовыйУровеньЛексемы > УровеньЛексемы Тогда // здесь должны быть равнозначные узлы. Например, сценарий/примеры
			...
			ЗаполнитьДеревоФич(НайденныеЛексемы, НоваяСтрока.Строки, Индекс, ПутьФайла);
		Иначе
			...
			Если НовыйУровеньЛексемы < УровеньЛексемыРодителя Тогда
				...
				ЗаполнитьДеревоФич(НайденныеЛексемы, Родитель.Строки, Индекс, ПутьФайла);
				Возврат;
			Иначе
				Пока Истина Цикл
					Если НовыйУровеньЛексемы <= УровеньЛексемыРодителя Тогда
						Лог.Отладка("Возвращаюсь на предыдущий уровень дерева фич");
						ЗаполнитьДеревоФич(НайденныеЛексемы, Родитель.Родитель.Строки, Индекс, ПутьФайла);
						Возврат;
					Иначе
