...
Функция РазобратьТекстФичи(Знач Фича)
	...
	Пока Истина Цикл
		...
		ИначеЕсли ПервыйСимвол = ВозможныеТипыНачалаСтроки.Таблица И НЕ ПредыдущиеПараметрыЛексемы.ДалееБудутДополнительныеСтроки Тогда
			Лог.Отладка("Очередная строка фичи <"+ОчереднаяСтрока+">");

			ПредыдущиеПараметрыЛексемы.ЕстьТелоТаблицы = Истина;
			ПредыдущиеПараметрыЛексемы.ТелоТаблицы = ПолучитьТелоТаблицыДанных(Фича, ОчереднаяСтрока);
			
			// ПраваяЧасть = ПредыдущиеПараметрыЛексемы.ПраваяЧасть + ?(ПустаяСтрока(ПредыдущиеПараметрыЛексемы.ПраваяЧасть), "", Символы.ПС);
			// ПредыдущиеПараметрыЛексемы.ПраваяЧасть = ПраваяЧасть + ПредыдущиеПараметрыЛексемы.ТелоТаблицы;
			Лог.Отладка("Добавляю таблицу данных как часть предыдущей лексемы. Тело <%1>", ПредыдущиеПараметрыЛексемы.ТелоТаблицы);
			Продолжить;
...
Функция ИзвлечьПараметры(Знач ПараметрыЛексемы, НайденныеПараметры, ПараметрыДляТаблицы, ЕстьПараметрыДляТаблицы)
	...
	Если ПараметрыЛексемы.ЕстьТелоТаблицы Тогда
		Тело = СтрШаблон("%1 %2", Тело, ИзвлечьПараметрТаблица(ПараметрыЛексемы.ТелоТаблицы, НайденныеПараметры));
...
Функция ПолучитьТаблицу(Знач ТекстТаблицы)
	...
	Ожидаем.Что(ЧислоСтрок, СтрШаблон("Количество строк из таблицы должно быть больше 0, а это не так. Текст %1", ТекстТаблицы)).Больше(0);
	...
	Для НомерСтроки = 2 По ЧислоСтрок Цикл
		...
		Если МассивЗначений = Неопределено Тогда
			Прервать;
		КонецЕсли;
		Ожидаем.Что(МассивЗначений, СтрШаблон("Получили неверное количество строк из таблицы. Текст %1", ТекстТаблицы)).ИмеетДлину(МассивЗначенийПервойСтроки.Количество());
...
Функция ПолучитьНаборЗначенийИзТекстовойСтрокиТаблицы(Знач ТекстТаблицы, Знач НомерСтроки)
	...
	Если НомерСтроки = 1 Тогда
		Лог.Отладка(СтрШаблон("Первая СтрокаПримеров %1", СтрокаТекстовойТаблицы));
	Иначе
		Лог.Отладка(СтрШаблон(" СтрокаПримеров №%2 %1", СтрокаТекстовойТаблицы, НомерСтроки-1));
	КонецЕсли;
...
Процедура ИспользоватьПервуюСтрокуКакИменаКолонокТаблицы(Таблица)
	Ожидаем.Что(Таблица.Количество(), "Количество строк из таблицы примеров должно быть больше 1, а это не так").Больше(1);
