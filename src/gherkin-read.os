...
Процедура ОбработатьДеревоФич(НайденныеЛексемы, СтрокиДерева, Индекс)
	Пока Индекс < НайденныеЛексемы.Количество() Цикл
		...
		НоваяСтрока = СоздатьОписаниеУзлаДереваФич(ОчереднаяЛексема, НоваяПраваяЧасть);
...
Функция СоздатьОписаниеУзлаДереваФич(Лексема = "", Тело = "")
	Возврат Новый Структура("Лексема,Тело,Строки,Параметры", Лексема, Тело, Новый Массив, Новый Массив); 
КонецФункции
