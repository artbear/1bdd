...
Функция ПолучитьДеревоФич(НайденныеЛексемы)
	ДеревоФич = СоздатьОписаниеУзлаДереваФич();
	Индекс = 0;
...
Процедура ОбработатьДеревоФич(НайденныеЛексемы, СтрокиДерева, Индекс)
	// Лог.Отладка("Перехожу на новый вложенный уровень");
	...
	Пока Индекс < НайденныеЛексемы.Количество() Цикл
		...
		// Лог.Отладка("ТипЗнч(ОчереднаяЛексема) "+ТипЗнч(ОчереднаяЛексема));
		...
		НоваяСтрока = СоздатьОписаниеУзлаДереваФич(ОчереднаяЛексема, ПараметрыОчереднойЛексемы.ПраваяЧасть);
		// НоваяСтрока= Новый Структура("Лексема,Тело,Строки", ОчереднаяЛексема, ПараметрыОчереднойЛексемы.ПраваяЧасть, Новый Массив); 
		// Лог.Отладка("НоваяСтрока.Лексема "+НоваяСтрока.Лексема);
		// Если ЗначениеЗаполнено(СтараяСтрока) Тогда
			// Лог.Отладка("СтараяСтрока.Лексема "+СтараяСтрока.Лексема);
		// КонецЕсли;
		СтараяСтрока = НоваяСтрока;
		СтрокиДерева.Добавить(НоваяСтрока);
		...
		Лог.Отладка("Проверяю следующую лексему <"+НоваяЛексема+">, Индекс "+Индекс+", уровень <"+НовыйУровеньЛексемы+">");//+", тело <"+ПараметрыОчереднойЛексемы.ПраваяЧасть+">");
...
Функция СоздатьОписаниеУзлаДереваФич(Лексема = "", Тело = "")
	// Возврат Новый Структура("Лексема,Тело,Строки", Лексема, Тело, Новый Массив); 
	
	Рез = Новый Структура("Строки", Новый Массив); 
