...
Процедура ПоказатьДеревоВРежимеОтладки(Узел, Знач Отступ = "", Знач ВыводитьШапку = Истина) Экспорт
	...
	Попытка
		Лог.Отладка(СтрШаблон("%4 тип шага %1, лексема %2, тело %3", 
				Узел.ТипШага, Узел.Лексема, Узел.Тело, Отступ));
		Родитель = Узел.Родитель;
		Лог.Отладка(СтрШаблон("%4     родитель тип шага %1, лексема %2, тело %3", 
				Родитель.ТипШага, Родитель.Лексема, Родитель.Тело, Отступ));
	Исключение
		// скрываем ошибки
	КонецПопытки;
