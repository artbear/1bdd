...
Функция ИзвлечьПараметры(Знач Тело, ПараметрыИзСтроки)
	...
	//Лог.Отладка("НовоеТело (после обработки чисел) <"+ НовоеТело+">");
	Возврат НовоеТело;
КонецФункции

Функция ВыделитьСтроковыеПараметры(Знач Тело, Параметры)
	Итератор = Найти(Тело,"'");
...
Функция ВыделитьЧисловыеПараметры(Знач Тело, Параметры)
	...
	НовоеТело = Тело;
	//Лог.Отладка("КоллекцияГруппСовпадений.Количество() "+КоллекцияГруппСовпадений.Количество());
