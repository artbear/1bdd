...
Функция ИзвлечьПараметры(Знач Тело, НайденныеПараметры, ПараметрыДляТаблицы, ЕстьПараметрыДляТаблицы)
	...
	НовоеТело = ВыделитьПараметрыИзТаблицы(НовоеТело, НайденныеПараметры, ЕстьПараметрыДляТаблицы);
...
Процедура ДобавитьПараметрыВОбратномПорядке(ТабНазначение, ТабИсточник)
	Для к = -ТабИсточник.Количество()+1 По 0 Цикл
		СтрокаПараметров = ТабИсточник[-к];
		ДобавитьПараметр(ТабНазначение, СтрокаПараметров.Тип, СтрокаПараметров.Значение, СтрокаПараметров.Начало);
		Лог.Отладка(СтрШаблон("ДобавитьПараметрыВОбратномПорядке %1, тип %2, начало %3 ", СтрокаПараметров.Значение, СтрокаПараметров.Тип, СтрокаПараметров.Начало));
	КонецЦикла;
КонецПроцедуры
