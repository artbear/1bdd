...
Процедура ЗаполнитьФайлШагов(ФайлФичи, РезультатыРазбора, ФайлШагов, НаборАдресовШагов)
	...
	Если Не ЭтоПервичнаяГенерация Тогда
		ТекстШагов = Новый ТекстовыйДокумент;
		ТекстШагов.Прочитать(ФайлШагов.ПолноеИмя);
	...
	Попытка
		Если ЭтоПервичнаяГенерация Тогда
			ЗаписатьПеременныеВТелоШага(ЗаписьФайла);
		КонецЕсли;
		ЗаписатьБазовоеТелоШага(ОписаниеЗаписываемогоФайла, ДеревоФич.Строки[0], НаборАдресовШагов);
...
Процедура ЗаписатьПеременныеВТелоШага(ЗаписьФайла)
	ЗаписьФайла.ЗаписатьСтроку("Перем БДД;");
	ЗаписьФайла.ЗаписатьСтроку("");
КонецПроцедуры

Процедура ЗаписатьБазовоеТелоШага(ОписаниеЗаписываемогоФайла, Узел, НаборАдресовШагов)
