...
Функция ВыполнитьФичу(Знач ФайлФичи, Знач ФайлБиблиотек = Неопределено, Знач ИскатьВПодкаталогах = Истина, 
	Знач ИспользоватьБыстрыйОстановНаОшибке = Ложь, Знач ИмяЭлементаСценария = "") Экспорт
...
Функция ДополнитьНаборШаговИзИсполнителяШаговФичи(Знач ФайлСценария, Знач НаборБиблиотечныхШагов)
	ОписаниеИсполнителяШагов = НайтиИсполнителяШагов(ФайлСценария, Истина);
...
// 		"Исполнитель" - объект-исполнитель шага (os-скрипт)
// 		"Файл" - объект-файл с информацией о файле-исполнителе шага
Функция НайтиИсполнителяШагов(Знач ФайлСценария, Знач ПоказыватьОшибкиИсполнителей = Ложь)
	...
	ОписаниеИсполнителя = ПолучитьИсполнителяШагов(ФайлИсполнителя, ПоказыватьОшибкиИсполнителей);
	...
	Если ФайлИсполнителя.Существует() Тогда
		...
		Исключение
			...
			Если ПоказыватьОшибкиИсполнителей Тогда
				Лог.Отладка("Ошибка при загрузке файла %1 %2%3", ФайлИсполнителя.ПолноеИмя, Символы.ПС, ПодробноеПредставлениеОшибки(Инфо));							
			КонецЕсли;
