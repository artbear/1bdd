...
Процедура ВыполнитьШаг(Знач НаименованиеШагаСценария) Экспорт
	...
	РезультатыВыполнения = ВыполнитьДеревоФич(РезультатыРазбора, мНаборБиблиотечныхШагов, Неопределено);
...
Процедура ВывестиИтоговыеРезультатыВыполнения(Знач МассивИтогов, Знач СтруктураИтогов, Знач ПоказыватьИтогиФич, Знач СтатусВыполнения) Экспорт
...
Функция ВыполнитьФичуСУчетомБиблиотечныхШагов(Знач ФайлФичи)
	...
	НовыйНаборБиблиотечныхШагов = ДополнитьНаборШаговИзИсполнителяШаговФичи(ФайлФичи, мНаборБиблиотечныхШагов, ОписаниеИсполнителяШагов);
...
Функция ВыполнитьДеревоФич(РезультатыРазбора, Знач НаборБиблиотечныхШагов, Знач ОписаниеИсполнителяШагов)
	...
	РезультатыВыполнения.Строки[0].СтатусВыполнения = РекурсивноВыполнитьШаги(ОписаниеИсполнителяШагов, НаборБиблиотечныхШагов, РезультатыВыполнения.Строки[0]);
	
	Возврат РезультатыВыполнения;
КонецФункции

Функция ДополнитьНаборШаговИзИсполнителяШаговФичи(Знач ФайлСценария, Знач НаборБиблиотечныхШагов, ОписаниеИсполнителяШагов)
	...
	Если ОписаниеИсполнителяШагов <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(НаборБиблиотечныхШагов) Тогда
			Для каждого КлючЗначение Из НаборШаговИсполнителя Цикл
				ДобавитьАдресШагаВКоллекциюШагов(НаборБиблиотечныхШагов, КлючЗначение.Ключ, КлючЗначение.Ключ, КлючЗначение.Значение, Истина);			
			КонецЦикла;
		Иначе
...
Процедура ДобавитьАдресШагаВКоллекциюШагов(КоллекцияШагов, Знач АдресШага, Знач ИмяШага, Знач ОписаниеИсполнителяШагов, Знач ВставлятьДублиШагов)
	ОписаниеИсполнителяШаговДляСравнения = Неопределено;
	КоллекцияШагов.Свойство(АдресШага, ОписаниеИсполнителяШаговДляСравнения);
	ВставлятьШаг = Истина;

	Если ОписаниеИсполнителяШаговДляСравнения <> Неопределено И ОписаниеИсполнителяШаговДляСравнения.Файл.ПолноеИмя <> ОписаниеИсполнителяШагов.Файл.ПолноеИмя Тогда
...
Функция НайтиИсполнителяШагов(Знач ФайлФичи)
	...
	Лог.Отладка("Ищу исполнителя шагов в каталоге "+ПутьФичи);
...
Функция ПолучитьИсполнителяШагов(Знач ФайлИсполнителя, Знач ПоказыватьОшибкиИсполнителей = Ложь)
	...
	Лог.Отладка("Ищу исполнителя шагов в файле "+ПутьИсполнителя);

	Если мКешИсполнителейШагов = Неопределено Тогда
		мКешИсполнителейШагов = Новый Соответствие();
	КонецЕсли;
	ОписаниеИсполнителя = мКешИсполнителейШагов.Получить(ПутьИсполнителя);
	Если ОписаниеИсполнителя = Неопределено Тогда
		...
		Если ФайлИсполнителя.Существует() Тогда
			Попытка
				...
				мКешИсполнителейШагов.Вставить(ПутьИсполнителя, ОписаниеИсполнителя);
			Исключение
				Инфо = ИнформацияОбОшибке();
				Если ПоказыватьОшибкиИсполнителей Тогда
					Лог.Предупреждение("Ошибка при загрузке файла %1 %2%3", ПутьИсполнителя, Символы.ПС, ПодробноеПредставлениеОшибки(Инфо));							
				КонецЕсли;
...
Функция РекурсивноВыполнитьШаги(Знач ОписаниеИсполнителяШагов, Знач НаборБиблиотечныхШагов, Знач Узел)
	...
	ХукВыполненУспешно = ВыполнитьХукУзла(ЧитательГеркин.ВозможныеХуки().ПередЗапускомСценария, ОписаниеИсполнителяШагов, Узел);
	...
	ХукВыполненУспешно = ВыполнитьХукУзла(ЧитательГеркин.ВозможныеХуки().ПослеЗапускаСценария, ОписаниеИсполнителяШагов, Узел);
	Если Не ХукВыполненУспешно Тогда
		СтатусВыполнения = ВозможныеСтатусыВыполнения.Сломался;
	КонецЕсли;
...
Функция ВыполнитьХукУзла(Знач ОписаниеХука, Знач ОписаниеИсполнителяШагов, Знач Узел, Знач ПредставлениеШага = "")
	...
	Если Узел.ТипШага = ОписаниеХука.ТипШага Тогда
		...
		Рефлектор = Новый Рефлектор;
		Если ОписаниеИсполнителяШагов <> Неопределено И Рефлектор.МетодСуществует(ОписаниеИсполнителяШагов.Исполнитель, АдресХука) Тогда
...
Функция ВыполнитьДействиеУзла(Знач НаборБиблиотечныхШагов, Знач Узел)
	...
	СтатусВыполнения = ВозможныеСтатусыВыполнения.НеВыполнялся;
	Если Узел.ТипШага = ВозможныеТипыШагов.Шаг Тогда
		СтатусВыполнения = ВыполнитьШагСценария(Узел.АдресШага, Узел.Параметры, НаборБиблиотечныхШагов, Узел.Тело, Узел.ОписаниеОшибкиВыполнения);
...
Функция ВыполнитьШагСценария(Знач АдресШага, Знач ПараметрыШага, Знач НаборБиблиотечныхШагов, Знач ПредставлениеШага, ОписаниеОшибкиВыполнения)
	СтатусВыполнения = ВозможныеСтатусыВыполнения.НеВыполнялся;
