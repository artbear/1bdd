...
Функция РекурсивноВыполнитьШаги(Знач ФайлСценария, Знач НаборБиблиотечныхШагов, Знач Узел)
	...
	ХукВыполненУспешно = ВыполнитьХукУзла(ВозможныеХуки.ПередЗапускомСценария, ФайлСценария, Узел);
	...
	ХукВыполненУспешно = ВыполнитьХукУзла(ВозможныеХуки.ПослеЗапускаСценария, ФайлСценария, Узел);
	Если Не ХукВыполненУспешно Тогда
		СтатусВыполнения = ВозможныеСтатусыВыполнения.Сломался;
	КонецЕсли;
