...
#Область ВыполнениеВКоманднойСтроке

// Выполнить фичи из командной строки
//
// Параметры:
//   ОписаниеПокрытия - Структура - <описание параметра>
//		* ИспользуетсяПокрытиеКода - Булево - включен расчет покрытия
//		* ПутьКаталогаФайловПокрытия - Строка - куда складывать результаты расчета покрытия
//   ПутьФич - Строка - путь к каталогу с фичами или к файлу фичи. Необязательно. По умолчанию "features"
//   ПутьОтчетаХмл - Строка - путь для хмл-отчета. Необязательно. По умолчанию "./bdd-log.xml"
//
//  Возвращаемое значение:
//   Булево - Успешное тестирование или нет
//
Функция ВыполнитьФичиИзКоманднойСтроки(Знач ОписаниеПокрытия, Знач ПутьФич = "features", Знач ПутьОтчетаХмл = "./bdd-log.xml") Экспорт
	...
	Если ОписаниеПокрытия.ИспользуетсяПокрытиеКода Тогда
		СохранитьВКонтекст("ПризнакСтатистикиСкриптовOnescript", 
			Новый Файл(ОписаниеПокрытия.ПутьКаталогаФайловПокрытия));
	КонецЕсли;

	РезультатыВыполнения = ВыполнитьФичу(Файл_КаталогФич, Файл_КаталогФич);
	ИтоговыйРезультатВыполнения = ПолучитьИтоговыйСтатусВыполнения(РезультатыВыполнения);
	...
	Если РезультатыВыполнения.Строки.Количество() > 0 Тогда
		...
		ВывестиИтоговыеРезультатыВыполнения(РезультатыВыполнения, Файл_КаталогФич.ЭтоКаталог());
	КонецЕсли;

	Если ОписаниеПокрытия.ИспользуетсяПокрытиеКода Тогда
		ГенераторОтчета = Новый ГенераторОтчетаSonarqubeGenExec;
	Иначе
		ГенераторОтчета = Новый ГенераторОтчетаJUnit;
	КонецЕсли;
	ГенераторОтчета.Сформировать(РезультатыВыполнения, СтатусВыполнения, ПутьОтчетаХмл);

	Сообщить(СтрШаблон("Результат прогона фич <%1>. Путь %2
	|", ИтоговыйРезультатВыполнения, ПутьФич));

	Возврат ИтоговыйРезультатВыполнения <> ВозможныеСтатусыВыполнения.Сломался;
