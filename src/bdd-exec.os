...
Функция ВыполнитьФичиИзКаталога(Знач ФайлКаталога, Знач ФайлБиблиотек = Неопределено, Знач ИскатьВПодкаталогах = Истина) Экспорт
	...
	Для каждого ФайлФичи Из МассивФайлов Цикл
		...
		КонецЕсли;
		РезультатВыполнения = ВыполнитьФичу(ФайлФичи, ФайлБиблиотек);
...
Функция ВыполнитьФичу(Знач ФайлФичи, Знач ФайлБиблиотек = Неопределено) Экспорт
...
Функция ПолучитьНаборБиблиотечныхШагов(Знач ФайлБиблиотек)
	...
	Для каждого Исполнитель Из МассивИсполнителейШагов Цикл
		...
		Для каждого ИмяШага Из МассивОписанийШагов Цикл
			Если КоллекцияШагов.Получить(ИмяШага) = Неопределено Тогда
				КоллекцияШагов.Вставить(ИмяШага, Исполнитель);
...
Функция ПолучитЬМассивИсполнителейШагов(Знач ФайлБиблиотек)
	...
	Если Не ФайлБиблиотек.ЭтоКаталог() Тогда
		МассивИсполнителейШагов.Добавить(НайтиИсполнителяШагов(ФайлБиблиотек));
	Иначе
		...
		Для каждого ФайлИсполнителя Из МассивФайлов Цикл
			...
			КонецЕсли;
			МассивИсполнителейШагов.Добавить(ПолучитьИсполнителяШагов(ФайлИсполнителя));
		КонецЦикла;
	КонецЕсли;
...
Функция ВыполнитьШаг(Знач АдресШага, Знач ПараметрыШага, Знач НаборБиблиотечныхШагов)
	...
	ИсполнительШагов = НаборБиблиотечныхШагов[АдресШага];
	Если ИсполнительШагов = Неопределено Тогда
