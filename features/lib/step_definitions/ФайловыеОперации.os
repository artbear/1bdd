...
Функция ПолучитьПутьФайлаСУчетомПеременныхКонтекста(Знач ПутьКаталога)
	Рез = ПутьКаталога;
	Лог.Отладка("ПолучитьПутьФайлаСУчетомПеременныхКонтекста %1", ПутьКаталога);
	// Файл = Новый Файл(ПутьКаталога);
	МассивИмен = СтрРазделить(СтрЗаменить(ПутьКаталога, "\", "/"), "/", Ложь);
	// МассивИмен = СтрРазделить(ПутьКаталога, "/\", Ложь);
	Если ЗначениеЗаполнено(МассивИмен) Тогда
		...
		ЗначениеИзКонтекста = БДД.ПолучитьИзКонтекста(ИмяКорня);
