...
Функция ФайлИлиКаталогСуществует(Знач ПутьФайла, ПолныйПуть, Знач ЭтоКаталог = Ложь, Знач ИсходныйКаталог = "")
	...
	Сообщить(СтрШаблон("ПутьФайла %1", ПутьФайла));
	Сообщить(СтрШаблон("ПолныйПуть %1", ПолныйПуть));
	Сообщить(СтрШаблон("ЭтоКаталог %1", ЭтоКаталог));
	Сообщить(СтрШаблон("ИсходныйКаталог %1", ИсходныйКаталог));

	Если ИсходныйКаталог = "" Тогда
		ИсходныйКаталог = ТекущийКаталог();
	КонецЕсли;
	Сообщить(СтрШаблон("Установили ИсходныйКаталог %1", ИсходныйКаталог));
	ПутьБезРегулярок = Истина;
	Попытка
		ПолныйПуть = (Новый Файл(ОбъединитьПути(ИсходныйКаталог, ПутьФайла))).ПолноеИмя;
		Сообщить(СтрШаблон("Установили ПолныйПуть %1", ПолныйПуть));
	Исключение
		ПутьБезРегулярок = Ложь;
		ПолныйПуть = СтрШаблон("%1/%2", ИсходныйКаталог, ПутьФайла); // в случае указания масок
		Сообщить(СтрШаблон("После исключения установили ПолныйПуть %1", ПолныйПуть));
	КонецПопытки;
	Сообщить(СтрШаблон("ПутьБезРегулярок %1", ПутьБезРегулярок));
	...
	Иначе		
		Попытка
			...
			Сообщить(СтрШаблон("МассивФайлов.Количество() %1", МассивФайлов.Количество()));
			Сообщить(СтрШаблон("Рез %1
			|	МассивФайлов.Количество() > 0", Рез));
		Исключение
			Сообщить(СтрШаблон("ОписаниеОшибки %1", ОписаниеОшибки()));
		КонецПопытки;
		Если Рез Тогда
			Объект = МассивФайлов[0];
			Сообщить(СтрШаблон("Объект %1", Объект.ПолноеИмя));
			Рез = ЭтоКаталог И Объект.ЭтоКаталог() Или Не ЭтоКаталог И Не Объект.ЭтоКаталог();
			Сообщить(СтрШаблон("Рез %1
			|	ЭтоКаталог И Объект.ЭтоКаталог() Или Не ЭтоКаталог И Не Объект.ЭтоКаталог()", Рез));
		КонецЕсли;
