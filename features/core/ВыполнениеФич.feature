...
Сценарий: Выполнение фич из каталога с использованием полного пути каталога
...
# TODO использовать только пока нет повторного использования шагов
  Когда я выполнил подключение тестового скрипта "ПроверкаГенерации"
	...
	И я подготовил специальную тестовую фичу "ПередачаПараметров"
	И я подставил файл шагов с уже реализованными шагами для фичи "ПередачаПараметров"
	Тогда проверка поведения фич из каталога "ТестовыйПолный" закончилась с кодом возврата 0

Сценарий: Выполнение фич из каталога с использованием относительного пути каталога
...
# TODO использовать только пока нет повторного использования шагов
  Когда я выполнил подключение тестового скрипта "ПроверкаГенерации"
	...
	И я подготовил специальную тестовую фичу "ПередачаПараметров"
	И я подставил файл шагов с уже реализованными шагами для фичи "ПередачаПараметров"
	Тогда проверка поведения фич из каталога "ТестовыйОтносительный" закончилась с кодом возврата 0
