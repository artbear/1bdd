...
Сценарий: При указании ключа быстрого останова на первой ошибке выполнение останавливается на первом же упавшем сценарии
	...
	И в лог-файле запуска продукта есть строка "2 Сценарий ( 0 Пройден, 0 Не реализован, 1 Сломался, 1 Не выполнялся )"
	И в лог-файле запуска продукта есть строка "4 Шаг ( 0 Пройден, 0 Не реализован, 1 Сломался, 3 Не выполнялся )"
...
Сценарий: Без указания ключа быстрого останова на первой ошибке выполнение не останавливается на первом же упавшем сценарии
	...
	И в лог-файле запуска продукта есть строка "2 Сценарий ( 0 Пройден, 0 Не реализован, 2 Сломался, 0 Не выполнялся )"
	И в лог-файле запуска продукта есть строка "4 Шаг ( 0 Пройден, 0 Не реализован, 2 Сломался, 2 Не выполнялся )"
