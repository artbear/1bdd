...
Процедура ПроверкаПоведенияФичиЗакончиласьСКодомВозврата(ИмяФичи, ОжидаемыйКодВозврата) Экспорт
	//ПроверкаПоведенияФичИзКаталогаЗакончиласьСКодомВозврата(ИмяФичи, ОжидаемыйКодВозврата);
	//Возврат;
	...
	Ожидаем.Что(КодВозврата, "ПроверкаПоведенияФичиЗакончиласьСКодомВозврата").Равно(ОжидаемыйКодВозврата);
КонецПроцедуры
...
Процедура ПроверкаПоведенияФичИзКаталогаЗакончиласьСКодомВозврата(ПутьКаталога, ОжидаемыйКодВозврата) Экспорт
	...
	ИмяЛогаКоманды = ОбъединитьПути(ВременныйКаталогФичи.ПолноеИмя, "exec-log.txt");
	СтрокаКоманды = СтрШаблон("cmd /c oscript.exe %4 %1 %2 -debug on %3", ПутьИсполнителяБДД, ПутьКаталога,
		СуффиксПеренаправленияВывода(ИмяЛогаКоманды, Ложь), "-encoding=utf-8");
	Лог.Отладка("СтрокаКоманды "+СтрокаКоманды);

	КодВозврата = Неопределено;
	ЗапуститьПриложение(СтрокаКоманды, ВременныйКаталогФичи.ПолноеИмя, Истина, КодВозврата);
